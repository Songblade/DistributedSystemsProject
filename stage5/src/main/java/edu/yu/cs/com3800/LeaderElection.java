...
public class LeaderElection {
    ...
    public LeaderElection(PeerServer server, LinkedBlockingQueue<Message> incomingMessages, Logger logger) {
        ...
        proposedEpoch = server.getPeerEpoch() + 1;
        // I moved this here, because otherwise, there could potentially be a race condition with
        ...
        if (server.getPeerState() == PeerServer.ServerState.OBSERVER) {
            ...
            // But I kept the epoch, because otherwise, we might recognize invalid votes as valid
    ...
    public synchronized Vote lookForLeader() {
        logger.info("Leader not found. Starting leader election voting for " + proposedLeader + " in epoch " + proposedEpoch);
        ...
        try {
            ...
            while(true){
                ...
                if (message == null) {
                    ...
                    for (int wait = 100; wait < maxNotificationInterval && message == null; wait *= 2) {
                        sendNotifications();
                        Thread.sleep(wait);
                    ...
                    // first, let's make sure that the peer isn't dead
                    // because if it is, we should ignore its vote
                    InetSocketAddress peerAddress = new InetSocketAddress(message.getSenderHost(), message.getSenderPort());
                    if (server.isPeerDead(peerAddress)) {
                    ...
                    if (notification.getState() != PeerServer.ServerState.OBSERVER && notification.getPeerEpoch() >= proposedEpoch) {
                        ...
                        logger.finer("Received valid vote " + notification + " from " + notification.getSenderID() + " in state " + notification.getState());

                        votes.put(notification.getSenderID(), notification);
                        // NOTE: The following is my personal addition, to solve the problem of latecomers to the election with a higher ID
                        // if we got here, than their epoch is at least as big as ours
                        // so if they are a leader or follower, then they must be right
                        // Yonatan Ginsberg said that you let him keep his solution, so I'm doing the same
                        if (notification.getState() == PeerServer.ServerState.LEADING || notification.getState() == PeerServer.ServerState.FOLLOWING) {
                            proposedLeader = notification.getProposedLeaderID();
                            proposedEpoch = notification.getPeerEpoch();
                            logger.fine("We received a vote from a server in state " + notification.getState() + " in epoch " + proposedEpoch + ", so the leader must be " + proposedLeader);
                            // there's no need to send notifications, the election is already over
                            // just declare the election over now
    ...
    private Vote acceptElectionWinner(ElectionNotification n) {
        ...
        /* I remove this so that I only set the state once, so no redundant log messages
        We do so when we return
        server.setPeerState(
    ...
    // I should be allowed to make this public, since I think I only made it protected based on minimum access principles
    // While he required it, the only requirement was the signature at that UDPMessageReceiver be able to get it
